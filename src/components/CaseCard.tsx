...
function hasFeedback(text: string) {
...
export default function CaseCard({ c }: { c: Case }) {
  ...
  const hasFB = hasFeedback(c.AIボイス精査);

  return (
    <div className={`rounded-2xl shadow-sm overflow-hidden border ${
      contracted ? "border-green-200" : "border-gray-100"
    } bg-white`}>
      ...
      {/* FBありバッジ */}
      {hasFB && !contracted && (
        <div className="bg-indigo-50 border-b border-indigo-100 px-4 py-2 flex items-center gap-1.5">
          <span className="text-indigo-600 text-xs font-bold">📝 AIフィードバックあり</span>
        </div>
      )}
      {hasFB && contracted && (
        <div className="bg-indigo-50 border-b border-indigo-100 px-4 py-2 flex items-center gap-1.5">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap mb-1.5">
              <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full ${
                contracted ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"
              }`}>
                {contracted ? "契約" : "未契約"}
              </span>
            ...
            {!contracted && c.未契約理由1 && (
              <p className="text-xs text-red-500 mt-1">
                未契約理由: {c.未契約理由1}
                {c.未契約理由2 && c.未契約理由2 !== c.未契約理由1 ? ` / ${c.未契約理由2}` : ""}
              </p>
            )}
          </div>
      ...
      {/* 詳細展開 */}
      {open && (
        <div className="px-4 pb-5 border-t border-gray-100 space-y-0">

          {/* 着地アプローチ（契約時） */}
          {contracted && c.着地アプローチ && (
            <div className="bg-green-50 rounded-xl p-3.5 mt-3 border border-green-200">
          ...
          {/* ボイスメモ */}
          {c.ボイスURL && (
            <div className="py-2.5 border-t border-gray-100 mt-1">
              <a
                href={c.ボイスURL}
                target="_blank"
